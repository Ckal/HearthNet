...
class TestBlobManifest:
    ...
    def test_manifest_no_filename(self):
        ...
        try:
            manifest = BlobManifest(
                cid="sha256:root", size_bytes=100, chunk_size_bytes=100, chunks=[], filename=None
            )
...
class TestManifestCid:
    ...
    def test_manifest_cid_empty_chunks(self):
        ...
        try:
            manifest = BlobManifest(
                cid="sha256:root", size_bytes=0, chunk_size_bytes=256, chunks=[], filename=None
            )
            cid = manifest_cid(manifest)
            assert isinstance(cid, str)
        except Exception:
...
class TestBlobEdgeCases:
    ...
    def test_chunk_ref_with_blake3_cid(self):
        ...
        try:
            ref = ChunkRef(index=0, cid="blake3:abcdef0123456789", size_bytes=256)
    ...
    def test_manifest_with_large_chunks_list(self):
        ...
        try:
            chunks = [ChunkRef(index=i, cid=f"sha256:{i:08x}", size_bytes=256) for i in range(100)]
            manifest = BlobManifest(
                cid="sha256:root",
                size_bytes=100 * 256,
                chunk_size_bytes=256,
